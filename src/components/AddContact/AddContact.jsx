...
import { AddContactWrapper } from './AddContact.styles';
...
const AddContact = () => {
  ...
  const onFinish = (values) => {
    localStorage.getItem("contactList") && JSON.parse(localStorage.getItem("contactList")).length>0 ?
      localStorage.setItem("contactList", JSON.stringify([...JSON.parse(localStorage.getItem("contactList")), { id: imageId, ...values, image: imageURL }])) : localStorage.setItem("contactList", JSON.stringify([{ id: imageId, ...values, image: imageURL }]))
    form.resetFields();
    resetImageState()
    messageApi.open({
      type: 'success',
  ...
  return (
    <AddContactWrapper>
      ...
      <Row justify='center' align='middle'>
        <Col span={10} >
          <div>
            <Avatar shape="square" size={256} icon={file
              ? <img
                src={
